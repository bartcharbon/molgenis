...
public class SemanticSearchServiceImplTest extends AbstractMolgenisSpringTest
{
	...
	public void testSearchHypertension() throws InterruptedException, ExecutionException
	{
		...
		when(ontologyService.findOntologyTerms(ontologies, ImmutableSet.of("history", "hypertens"), 100))
	...
	public void testDistanceFrom()
	{
		...
		Assert.assertEquals(semanticSearchService
						.distanceFrom("Maternal Hypertension", ImmutableSet.of("history", "hypertens"), stemmer), .5454, 0.0001,
	...
	public void testSearchDescription() throws InterruptedException, ExecutionException
	{
		...
		when(ontologyService.findOntologyTerms(ontologies, ImmutableSet.of("standing", "height", "meters"), 100))
				.thenReturn(ontologyTerms);
		Hit<OntologyTerm> result = semanticSearchService.findTags(attribute, ontologies);
	...
	public void testSearchLabel() throws InterruptedException, ExecutionException
	{
		...
		when(ontologyService.findOntologyTerms(ontologies, ImmutableSet.of("standing", "height", "m"), 100))
				.thenReturn(ontologyTerms);
		Hit<OntologyTerm> result = semanticSearchService.findTags(attribute, ontologies);
	...
	public void testIsSingleMatchHighQuality()
	{
		...
		assertFalse(semanticSearchService
				.isSingleMatchHighQuality(newHashSet("height"), newHashSet("height"), explanations1));

		List<ExplainedQueryString> explanations2 = singletonList(
				ExplainedQueryString.create("body length", "body length", "height", 100));

		assertTrue(semanticSearchService
				.isSingleMatchHighQuality(newHashSet("height in meter"), newHashSet("height in meter", "height"),
						explanations2));
	...
	public void testFindAttributes()
	{
		...
		when(semanticSearchServiceHelper.createDisMaxQueryRuleForAttribute(newHashSet("targetAttribute"), emptyList()))
	...
	public void testConvertAttribute()
	{
		...
		when(elasticSearchExplainService.findQueriesFromExplanation(collectExpandedQueryMap, explanation))
